# Used under MIT license from ChristophH/Inferelator
# 
# Author: Christoph H
###############################################################################
...
BBSR <- function(X, Y, clr.mat, nS, no.pr.val, weights.mat, prior.mat, cores) {
  ...
  # All matrices need row and column names
  # X = regulators, regulators are rows
  # Y = genes, genes are rows
  # clr.mat = CLR results, rows are genes, cols are TFs 
  # nS = number of predictors to use per gene
  # no.pr.val = Value to indicate a missing pr(???)
  # weights.mat = rows are genes, cols are TFs
  # prior.mat = priors, rows are genes, cols are TFs
  # cores = # of cores to use
