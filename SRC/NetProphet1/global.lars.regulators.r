...
lars.multi <- function(tdata,rdata,pert,prior,allowed)
{
	lars.paths <- list()
		...
		cat("Running multi cv in parallel threads")
		cat("\n")
		cat("Building path for target: ")
		lars.paths <- future_lapply(targets, function(i)
				{
				cat(i,"")
...
# results.lars <- lars(t(x),tdata[i,mindices],trace=FALSE,max.steps=600,type="lasso",normalize=FALSE,intercept=TRUE,use.Gram=FALSE);
				...
				results.lars <- try(lars(t(x[,1:sample.indx]),
							y[1:sample.indx],
							trace=FALSE,
							max.steps=600,
							type="lasso",
...
#####
				return(list(beta=results.lars$beta, 
							mu=results.lars$mu, 
							meanx=results.lars$meanx, 
							C.Max=c(results.lars$C.Max,0)))
				})#, future.stdout=NA)
  cat("\n")
  ...
  for (i in targets)
  {
    all.C.lars <- rbind(all.C.lars, 
           cbind(lars.paths[[i]]$C.Max,
              rep(i,times=length(lars.paths[[i]]$C.Max)),
               1:length(lars.paths[[i]]$C.Max)))
  }
...
{
  cat("Running single cv in parallel threads")
  targets <- seq(dim(tdata)[1])
  B <- matrix(0,nrow=dim(rdata)[1],ncol=dim(tdata))

  minFrac <- c()
  cat("Building path for target: ")
  B <- future_lapply(targets, function(i) 
  {
    cat(i,"")
    mindices <- which(pert[i,]==0)
    x <- rdata[,mindices] * prior[,i]
    x[which(allowed[,i]==0),]<-0;
    set.seed(seed)
    lars.paths.cv <-  cv.lars(t(x),tdata[i,mindices],K=fold,plot.it=FALSE,trace=FALSE,max.steps=600,type="lasso",normalize=FALSE,intercept=TRUE,se=FALSE);
    ...
    beta <- predict(lars.paths,s=minFrac,type="coefficients",mode="fraction")$coefficients
    return(beta)
  })
